...
const DeviceBoard: React.FC<DeviceBoardProps> = ({ isOnline = false, deviceName = 'None' }) => {
  ...
  return (
    <View style={styles.container}>
        <View style={styles.dateTime}>
            ...
            >
                {`${date.time.hours.toString().padStart(2, '0')}:${date.time.minutes.toString().padStart(2, '0')}:${date.time.seconds.toString().padStart(2, '0')} ${date.isDaytime}`}
            </Text>
        </View>
        <View style={styles.line} />
        <View style={styles.device}>
            <View style={styles.statusTitle}>
                <View style={[styles.circleStatus, { backgroundColor: isOnline ? '#3FDE7F' : '#FF3D00' }]} />
                <Text
                    style={[styles.deviceStatus, styles.font]}
                >{ isOnline ? `Online` : `Offline`}</Text>
            </View>
            <Text
                style={[styles.deviceTitle, styles.font]}
            >{`Device`}</Text>
            <Text
